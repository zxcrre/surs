...
void main() {
    ...
    uv += WarpAmount * vec2(sin(uv.y * 6.283 + t * 0.5), cos(uv.x * 6.283 + t * 0.5)) * 0.05;
    
    vec4 col = cppn_fn(uv, 0.1 * sin(0.3 * t), 0.1 * sin(0.69 * t), 0.1 * sin(0.44 * t));
    
    col.rgb = hueShiftRGB(col.rgb, HueShift);
    
    float lum = dot(col.rgb, vec3(0.299, 0.587, 0.114));
    col.rgb = mix(col.rgb, TintColor * lum * 2.0, TintStrength);
    
    float scanline_val = sin(gl_FragCoord.y * ScanlineFrequency) * 0.5 + 0.5;
    col.rgb *= 1.0 - (scanline_val * scanline_val) * ScanlineIntensity;
    
    col.rgb += (rand(gl_FragCoord.xy + t) - 0.5) * NoiseIntensity;
    
    float fadeMask = 1.0;
    if (FadeHeight > 0.0) {
        float normalizedY = texCoord.y;
        fadeMask = smoothstep(1.0 - FadeHeight, 1.0, normalizedY);
    }
    
    vec4 original = texture(DiffuseSampler, texCoord);
    vec3 finalColor = mix(original.rgb, clamp(col.rgb, 0.0, 1.0), fadeMask * Opacity);
